...
class WikiParser {
          ...
          switch (anchor.substring(0, 1)) {
            ...
            case "]":
              if (type == "simpleBracket") {
                let inside = this.manageSimpleLink(
                  str.substring(lastAnchor - 1, cursor)
                );
                str =
                  str.substring(0, lastAnchor - 2) +
                  inside +
  ...
  manageTemplate(str) {
    ...
    if (wt !== undefined) {
      // Subtemplate ?
      if (wt.subTemplate) {
        let displayed = wt.subTemplate.displayed;
